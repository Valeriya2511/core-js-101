...
function timeSpanToString(startDate, endDate) {
  ...
  return `${arr.map((el) => el.toString().padStart(2, '0')).join(':')}.${milliseconds.toString().padStart(3, '0')}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const deg = Math.abs(hours * 30 + minutes * 0.5 - minutes * 6);
  const min = Math.min(360 - deg, deg);
